...
import { TextField, InputLabel, Table, TableBody, TableCell, TableRow, Button, Grid, FormControl, Select, MenuItem, SelectChangeEvent, List, ListItem, ListItemText, TableContainer, Paper } from '@mui/material';
...
const AddAgent = (props: any) => {
  ...
  const handleFileChange = (event: any) => {
    ...
    if (!validTypes.includes(event.target.files[0].type)) {
      ...
      if (event.target.name == "aadhar_card" || event.target.name == "pan_card" || event.target.name == "cancelled_cheque") {
  ...
  const handleSubmit = () => {
    ...
    // }
    axios.post(urls["dummyURL"], postObject)//Should ['submitURL'] else we can use ['dummyURL']
      .then(function (response) {
        setLoading(false);
        if (response.data.completed) {
          // console.log(response.data);
          resetForm()
          toast.success("Successfully created Agent - " + response.data.agent_code, { position: "top-center", theme: "colored", autoClose: 2000, hideProgressBar: true });
        }
        else {
          console.log("goes in else", response.data);

          setErrorMessage(prevErrorMessage => [...prevErrorMessage, "Error while creating Agent"]);
        };
      })
  ...
  useEffect(() => {
    if (errorMessage.length > 0) {
      toast.error(<div>Please fill the required fields.<br />{errorMessage.map((error) => <ul key={error}><li style={{ textAlign: "left" }}>{error}</li></ul>)}<br /></div>, { position: "top-right", autoClose: false, theme: "colored", hideProgressBar: false, closeOnClick: true, pauseOnHover: true, draggable: true, progress: undefined, });
    }
  }, [errorMessage]);
            ...
            <Paper sx={{ width: '100%' }}>
              <div className='table-data'>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <SupportAgentIcon color='primary' style={{ fontSize: '3rem' }} />
                  ...
                  <Table sx={{ minWidth: 650 }} aria-label="simple table">
                    <TableBody>
                      <TableRow>
                        ...
                        <TableCell colSpan={3}>
                          <TextField type="text" name="Agent Name" value={formData["agentName"]} onChange={(e) =>
                      ...
                      <TableRow>
                        ...
                        <TableCell>
                          <FormControl sx={{ minWidth: 220 }}>
                            ...
                            <Select label="State" value={formData["agentState"]} MenuProps={{ style: { maxHeight: 300 } }} onChange={(e) => handleFormChange("agentState", e.target.value)}>
                      ...
                      <TableRow>
                        ...
                        <TableCell>
                          <FormControl sx={{ minWidth: 220 }}>
                            ...
                            <Select label="Branch" value={formData["agentBranch"]} MenuProps={{ style: { maxHeight: 300 } }} onChange={(e) => handleFormChange("agentBranch", e.target.value)} disabled={formData["agentState"] == ""}>
                              {Object.keys(agentStateOptions[formData["agentState"]] ? agentStateOptions[formData["agentState"]] : []).map((option) => (
                                <MenuItem key={option} value={option}>
                                  {option}
                                </MenuItem>
                              ))}
                      ...
                      <TableRow>
                        ...
                        <TableCell>
                          <TextField type="Number" name="Agent Mobile Number" value={formData["agentMobileNumber"]} onChange={(e) =>
                            handleFormChange("agentMobileNumber", e.target.value)} />
                        </TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell>Address *</TableCell>
                        <TableCell>
                          <TextField type="text" name="Agent Address" value={formData["agentAddress"]} onChange={(e) =>
                      ...
                      <TableRow>
                        ...
                        <TableCell>
                          <input type="file" name='aadhar_card' onChange={(e) => handleFileChange(e)} />
                        </TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell>Pan card *</TableCell>
                        <TableCell>
                          <input type="file" name='pan_card' onChange={(e) => handleFileChange(e)} />
                        </TableCell>
                      </TableRow>
                      <TableRow>
                        <TableCell>Bank Account No.*</TableCell>
                        <TableCell>
                          <TextField type="text" name="Bank AcNo" value={formData["bankAccountNumber"]} onChange={(e) =>
                      ...
                      <TableRow>
                        ...
                        <TableCell>
                          <TextField type="text" name="Bank IFSC" value={formData["bankIfsc"]} onChange={(e) =>
                      ...
                      <TableRow>
                        ...
                        <TableCell>
                          <TextField type="text" name="Bank Branch" value={formData["bankBranchName"]} onChange={(e) =>
                      ...
                      <TableRow>
                        ...
                        <TableCell>
                          <input type="file" name='cancelled_cheque' onChange={(e) => handleFileChange(e)} />
                        </TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
                ...
                <br />
                <Button variant="contained" onClick={() => validateFeilds()}>Validate</Button>&nbsp;
