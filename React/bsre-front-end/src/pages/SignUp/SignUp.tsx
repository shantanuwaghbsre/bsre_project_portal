...
const Signup = (props: any) => {
      ...
      axios
        ...
        .then((response) => {
          ...
          if (response.data.completed) {
            resetForm();
            toast.success(
              `Successfully created user ${response.data.agent_code}`,
              {
                position: "top-center",
                theme: "colored",
                autoClose: 2000,
  ...
  const validateFields = () => {
    ...
    else if (!formData["agentState"])
      errorMessages.push("Agent's State is required");
    else if (!formData["agentBranch"])
      errorMessages.push("Agent's Branch is required");
    else if (!formData["agentMobileNumber"])
      errorMessages.push("Agent's Mobile Number is required");
    else if (!formData["agentAddress"])
      errorMessages.push("Agent's Address is required");
    else if (!formData["bankAccountNumber"])
      errorMessages.push("Agent's Bank Account Number is required");
    else if (!formData["bankIfsc"])
      errorMessages.push("Agent's Bank else ifSC is required");
    else if (!formData["bankBranchName"])
      errorMessages.push("Agent's Bank branch is required");
    else if (!files["aadhar_card"])
      errorMessages.push("Please upload Aadhar card");
    else if (!files["pan_card"]) errorMessages.push("Please upload Pan card");
    else if (!files["cancelled_cheque"])
      errorMessages.push("Please upload cancelled cheque");
          ...
          >
            ...
            <Stack spacing={2}>
              ...
              <FormControl sx={{ width: "400px" }}>
                ...
                >
                  {(agentStateOptions[formData["agentState"]] || []).map(
                    (option) => (
